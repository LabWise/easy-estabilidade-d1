...
-- Atualizar função para tratar período "Micro" como 6 meses
...
BEGIN
  ...
  LOOP
    -- Tratar períodos especiais
    IF periodo_record.periodo = 'Micro' THEN
      -- Período "Micro" é tratado como 6 meses
      meses_extraidos := 6;
      data_programada := p_data_entrada + (meses_extraidos * INTERVAL '1 month');
    ELSIF periodo_record.periodo ~ '\d+M' THEN
      -- Extrair o número de meses do período (ex: "3M" -> 3)
      meses_extraidos := CAST(substring(periodo_record.periodo from '\d+') AS INTEGER);
      data_programada := p_data_entrada + (meses_extraidos * INTERVAL '1 month');
    ELSE
      -- Fallback para períodos que não seguem o padrão "XM" ou não são especiais
